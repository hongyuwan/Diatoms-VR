...
public partial class RealtimeClient : MonoBehaviour
{
    ...
    // --- New: Instruction controls ---
    [Header("Assistant Instructions")]
    [SerializeField, TextArea(3, 12)] private string customInstructions =
        "Welcome to the underground sea with diatoms. I'm your friendly guide at the amazing world. " +
        "Ask me about the objects floating in the bubbles, and I'll keep it brief and engaging.";
    [SerializeField] private bool overrideInstructions = true;           // If true, replace config.instructions with customInstructions
    [SerializeField] private bool includeMuseumObjects = true;           // If true, append objects from ContentProvider
    [SerializeField] private bool logBuiltInstructions = false;          // If true, Debug.Log the final instructions
    ...
    public async Task ConnectAsync()
    {
        ...
        // Snapshot initial instructions then optionally override with custom text
        _initialConfigInstruction = config.instructions;
        if (overrideInstructions && !string.IsNullOrWhiteSpace(customInstructions))
            config.instructions = customInstructions.Trim();
        
        //Add included objects from the ContentProvider to the config instructions
        if (_contentProvider != null && includeMuseumObjects)
    ...
    {
        // Initialize a StringBuilder with the existing (possibly overridden) instructions.
